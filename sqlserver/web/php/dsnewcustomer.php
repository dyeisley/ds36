...
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country)
  ...
  $zip = empty($zip) ? 0 : intval($zip);
  $age = empty($age) ? 0 : intval($age);
  $income = empty($income) ? 0 : intval($income);
  $creditcardtype = empty($creditcardtype) ? 0 : intval($creditcardtype);

  $username_out = str_repeat(' ', 50);
  $sql = "SET NOCOUNT ON; EXEC NEW_CUSTOMER$storenum @firstname_in = ?, @lastname_in = ?, @address1_in = ?, " .
    "@address2_in = ?, @city_in = ?, @state_in = ?, @zip_in = ?, @country_in = ?, " .
    "@region_in = ?, @email_in = ?, @phone_in = ?, @creditcardtype_in = ?, " .
    "@creditcard_in = ?, @creditcardexpiration_in = ?, @username_out = ?, " .
    "@password_in = ?, @age_in = ?, @income_in = ?, @gender_in = ?";

  $params = array(
    $firstname, $lastname, $address1, $address2, $city, $state,
    array($zip, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT),
    $country,
    array($region, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_TINYINT),
    $email, $phone,
    array($creditcardtype, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_TINYINT),
    $creditcard, $creditcardexpiration,
    array(&$username_out, SQLSRV_PARAM_INOUT, null, SQLSRV_SQLTYPE_VARCHAR(50)),
    $password,
    array($age, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_TINYINT),
    array($income, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT),
    $gender
  );

  $stmt = sqlsrv_query($conn, $sql, $params);
  if ($stmt === false) {
    die("NEW_CUSTOMER procedure failed: " . print_r(sqlsrv_errors(), true));
  }
  ...
  echo "<H2>New Customer Successfully Added<H2>\n";
  echo "<P>Your username is: <strong>$username</strong></P>\n";
  echo "<P>Click below to begin shopping</P>\n";
  ...
  dsnewcustomer_form($firstname,$lastname,$address1,$address2,$city,$state,$zip,$country,
    $email,$phone,$creditcardtype,$creditcard,$ccexpmon,$ccexpyr,$password,$age,$income,$gender,$storenum);
  }
...
function dsnewcustomer_form($firstname,$lastname,$address1,$address2,$city,$state,$zip,$country,
  ...
  echo "<em>Note: Username will be automatically generated</em><BR>\n";
