 ...
 *
 * Creates new entry in SQL Server DVD Store REVIEWS table
 *
...
if (!(empty($productid) OR empty($review_stars) OR empty($review_summary) OR empty($review_text)))
  ...
  $sql = "SET NOCOUNT ON; EXEC NEW_PROD_REVIEW$storenum @prod_id_in = ?, @stars_in = ?, " .
    "@customerid_in = ?, @review_summary_in = ?, @review_text_in = ?";
  $params = array(
  ...
  if ($stmt === false) {
    die("NEW_PROD_REVIEW failed: " . print_r(sqlsrv_errors(), true));
  }

  $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_NUMERIC);
  $reviewid = $row ? $row[0] : 0;
  ...
  {
  $conn = ds_connect();
  $sql = "SELECT TITLE FROM PRODUCTS$storenum WHERE PROD_ID = ?";
  $stmt = sqlsrv_query($conn, $sql, array(array(intval($productid), SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT)));
  $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_NUMERIC);
  $producttitle = $row[0];
  sqlsrv_free_stmt($stmt);
  sqlsrv_close($conn);
