...
# Syntax to run - perl sqlserver_ds_perl_create_indexes_multi.pl <sqlserver_target> <number_of_stores> <password>
...
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">$sqlservertargetdir${pathsep}sqlserver_ds_createindexes$k.sql") || die("Can't open sqlserver_ds_indexes$k.sql");
	print $OUT "USE DS3
GO
...
CREATE INDEX IX_PROD_SPECIAL_CATEGORY_MEMBERSHIP$k ON PRODUCTS$k
  (
  SPECIAL ASC,
  CATEGORY ASC,
  MEMBERSHIP_ITEM ASC,
...
ALTER TABLE INVENTORY$k
  ADD CONSTRAINT FK_INVENTORY_PROD_ID$k FOREIGN KEY (PROD_ID)
...
CREATE INDEX IX_REVIEWS_PRODSTARS$k ON REVIEWS$k
  ...
  INCLUDE (REVIEW_ID, REVIEW_DATE, CUSTOMERID, REVIEW_SUMMARY)
  ON DS_IND_FG
GO
...
CREATE NONCLUSTERED INDEX IX_REVIEWS_PRODID_DATE$k ON REVIEWS$k
  (
  PROD_ID ASC,
  REVIEW_DATE DESC
  )
  INCLUDE (REVIEW_ID, STARS, CUSTOMERID, REVIEW_SUMMARY)
  WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, ONLINE = OFF)
  ON DS_IND_FG
go

CREATE INDEX IX_REVIEWS_PROD_HELPFULNESS$k ON REVIEWS$k
  (
  PROD_ID,
  TOTAL_HELPFULNESS DESC
  )
...
{
print $OUT "
IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'idx_product_vector$k')
    DROP INDEX idx_product_vector$k ON PRODUCTS$k;
GO
