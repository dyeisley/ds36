...
Design and requirements by Daniel Yeisley
"""
...
class DVDStoreGUI:
            ...
            for item in os.listdir(self.ds36_path):
                ...
                if os.path.isdir(item_path):
                    ...
                    if os.path.exists(script_path):
                        ...
                        if info:
                            # Known database - check for specific client tool
                            ...
                            if client_tool and self._is_client_tool_installed(client_tool):
                                # Found client tool specified in script
                                databases.append(item.upper())
                            else:
                                # No client tool specified or not installed - try common patterns
                                client_candidates = [item, f"{item}sql", f"hdb{item}"]
                                for candidate in client_candidates:
                                    if self._is_client_tool_installed(candidate):
                                        databases.append(item.upper())
                                        break
                            # If no client tool found, database won't appear in Step 2
        except Exception as e:
    ...
    def _parse_script_syntax(self, db_dir, script_path):
        """Parse script header to determine parameter requirements and client tool

        Looks for lines like:
        # syntax is: script_name.sh <target> <stores> <password> [use_vectors]
        # Assumes hdbsql is in PATH.
        """
            ...
            with open(script_path, 'r') as f:
                ...
                for line in f:
                    ...
                    if '# syntax' in line.lower() or '# usage' in line.lower():
                        ...
                        # Check if use_vectors is present
                        supports_vectors = 'use_vectors' in line.lower() or 'use-vectors' in line.lower()

                    # Look for client tool requirement
                    # Patterns: "Assumes X is in PATH" or "Requires X"
                    if 'assumes' in line.lower() or 'requires' in line.lower():
                        # Extract tool name (usually before "is in PATH" or standalone)
                        if 'is in path' in line.lower():
                            # Extract word before "is in PATH"
                            parts = line.lower().split('is in path')
                            if parts:
                                words = parts[0].strip().split()
                                if words:
                                    client_tool = words[-1]  # Last word before "is in PATH"

                    # Stop scanning after first 20 lines
                    if line_count > 20:
                        break
        except Exception as e:
            # Defaults on error
