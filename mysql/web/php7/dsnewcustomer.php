...
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country)
  ...
  // NEW_CUSTOMER generates username (OUT parameter) and customerid (OUT parameter)
  ...
  if (!mysqli_multi_query($link_id, $new_customer_proc_call)) {
    die("NEW_CUSTOMER procedure failed: " . mysqli_error($link_id));
  }
  ...
  // Get output parameters (username and customerid)
  $query = "select @username_out, @customerid_out;";
