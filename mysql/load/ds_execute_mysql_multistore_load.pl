...
system ("mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < mysql_ds_disable_redo_log.sql");
...
if ("$^O" eq "linux")
	...
	chdir("$base_dir\\membership\\$mysql_targetdir");
	foreach my $k (1 .. $numberofstores){
		system ("start remote_mysql_ds_membership_load$k.bat");
		}

	chdir("$base_dir\\prod\\$mysql_targetdir");
	foreach my $k (1 .. $numberofstores){
		system ("start remote_mysql_ds_prod_load$k.bat");
		system ("start remote_mysql_ds_inv_load$k.bat");
		}
	
	chdir("$base_dir\\reviews\\$mysql_targetdir");
	foreach my $k (1 .. $numberofstores){
		system ("start remote_mysql_ds_reviews_load$k.bat");
		system ("start remote_mysql_ds_reviewshelpful_load$k.bat");
		}
	
	chdir("$base_dir\\orders\\$mysql_targetdir");
	foreach my $k (1 .. $numberofstores){
		system ("start remote_mysql_ds_orders_load$k.bat");
		system ("start remote_mysql_ds_orderlines_load$k.bat");
		system ("start remote_mysql_ds_cust_hist_load$k.bat");
		}
	
	chdir("$base_dir\\cust\\$mysql_targetdir");
	foreach my $k (1 .. $numberofstores){
		system ("start remote_mysql_ds_cust_load$k.bat");
		}
...
system ("mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < mysql_ds_enable_redo_log.sql");
