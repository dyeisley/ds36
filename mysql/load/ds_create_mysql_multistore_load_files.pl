...
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">cust$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_load$k.sql") || die("Can't open cust$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
...
\n";
	close $OUT;
	open (my $OUTBAT, ">cust$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_load$k.bat") || die("Can't open cust$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_cust_load$k.sql\n";
	print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_cust_load$k.bat cust$pathsep");
	#system ("$movecommand remote_mysql_ds_cust_load$k.sql cust$pathsep");
}

# Orders 
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orders_load$k.sql") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orders_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
\n";
	close $OUT;
	open (my $OUTBAT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orders_load$k.bat") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orders_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_orders_load$k.sql\n";
	#print $OUT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_orders_load$k.bat orders$pathsep");
	#system ("$movecommand remote_mysql_ds_orders_load$k.sql orders$pathsep");
}

# Orderlines 
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orderlines_load$k.sql") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orderlines_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orderlines_load$k.bat") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_orderlines_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_orderlines_load$k.sql\n";
	#print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_orderlines_load$k.bat orders$pathsep");
	#system ("$movecommand remote_mysql_ds_orderlines_load$k.sql orders$pathsep");
}
# cust_hist 
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_hist_load$k.sql") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_hist_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
\n";
	close $OUT;
	open (my $OUTBAT, ">orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_hist_load$k.bat") || die("Can't open orders$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_cust_hist_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_cust_hist_load$k.sql\n";
	print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_cust_hist_load$k.bat orders$pathsep");
	#system ("$movecommand remote_mysql_ds_cust_hist_load$k.sql orders$pathsep");
}
# prod
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_prod_load$k.sql") || die("Can't open prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_prod_load$k.sql");
	print $OUT "use DS3;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_prod_load$k.bat") || die("Can't open prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_prod_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_prod_load$k.sql\n";
	print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_prod_load$k.bat prod$pathsep");
	#system ("$movecommand remote_mysql_ds_prod_load$k.sql prod$pathsep");
}
# inv
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_inv_load$k.sql") || die("Can't open prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_inv_load$k.sql");
	print $OUT "use DS3;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_inv_load$k.bat") || die("Can't open prod$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_inv_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_inv_load$k.sql\n";
	#print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_inv_load$k.bat prod$pathsep");
	#system ("$movecommand remote_mysql_ds_inv_load$k.sql prod$pathsep");
}
#membership
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">membership$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_membership_load$k.sql") || die("Can't open membership$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_membership_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">membership$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_membership_load$k.bat") || die("Can't open membership$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_membership_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_membership_load$k.sql\n";
	print $OUTBAT "$timecommand > finished$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_membership_load$k.sql membership$pathsep");
	#system ("$movecommand remote_mysql_ds_membership_load$k.bat membership$pathsep");
}
#reviews
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviews_load$k.sql") || die("Can't open reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviews_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviews_load$k.bat") || die("Can't open reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviews_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_reviews_load$k.sql\n";
	print $OUTBAT "$timecommand > finishedreview$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_reviews_load$k.bat reviews$pathsep");
	#system ("$movecommand remote_mysql_ds_reviews_load$k.sql reviews$pathsep");
}


#reviews helpfulness
foreach my $k (1 .. $numberofstores){
	open (my $OUT, ">reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviewshelpful_load$k.sql") || die("Can't open reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviewshelpful_load$k.sql");
	print $OUT "use DS3;
SET UNIQUE_CHECKS=0;
SET FOREIGN_KEY_CHECKS=0;
...
SET FOREIGN_KEY_CHECKS=1;\n";
	close $OUT;
	open (my $OUTBAT, ">reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviewshelpful_load$k.bat") || die("Can't open reviews$pathsep$mysql_targetdir${pathsep}remote_mysql_ds_reviewshelpful_load$k.bat");
	print $OUTBAT "mariadb -h $mysqltarget -u web --password=web --local_infile DS3 < remote_mysql_ds_reviewshelpful_load$k.sql\n";
	print $OUTBAT "$timecommand > finishedhelp$k.txt\n";
	print $OUTBAT "exit\n";
	close $OUTBAT;
	sleep(.1);
	#system ("$movecommand remote_mysql_ds_reviewshelpful_load$k.bat reviews$pathsep");
	#system ("$movecommand remote_mysql_ds_reviewshelpful_load$k.sql reviews$pathsep");
}
