# mysql_ds_perl_cleanup_multi.pl
# Script to cleanup or reset ds3 tables in MySQL with a provided number of copies - supporting multiple stores
# Syntax to run - perl mysql_ds_perl_cleanup_multi.pl <mysql_target> <number_of_stores> <DB Size - example 10> <DB Size Units - MB or GB>
...
foreach my $k (1 .. $numberofstores){
        open (my $OUT, ">mysql_ds_cleanuptables.sql") || die("Can't open mysql_ds_cleanuptables.sql");
...
\n";
  ...
  print ("mysql -h $mysqltarget -u web --password=web < mysql_ds_cleanuptables.sql\n");
  system ("mysql -h $mysqltarget -u web --password=web < mysql_ds_cleanuptables.sql");
  print ("Recreating Triggers\n");
  print ("mysql -h $mysqltarget -u web --password=web < $mysqltarget${pathsep}mysql_ds_create_trigger.sql\n");
  system ("mysql -h $mysqltarget -u web --password=web < $mysqltarget${pathsep}mysql_ds_create_trigger.sql");
  }
