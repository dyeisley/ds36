...
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country)
  ...
  // Convert empty numeric fields to NULL for PostgreSQL
  $zip = empty($zip) ? 'NULL' : "'" . intval($zip) . "'";
  $age = empty($age) ? 'NULL' : intval($age);
  $income = empty($income) ? 'NULL' : intval($income);
  $creditcardtype = empty($creditcardtype) ? 'NULL' : intval($creditcardtype);

  // new_customer generates username and returns (username, customerid)
  // Cast parameters to match function signature
  $new_customer_proc_call = "SELECT * FROM new_customer$storenum(" .
    "'$firstname'::VARCHAR, '$lastname'::VARCHAR, '$address1'::VARCHAR, '$address2'::VARCHAR, " .
    "'$city'::VARCHAR, '$state'::VARCHAR, $zip::VARCHAR, '$country'::VARCHAR, " .
    "$region::SMALLINT, '$email'::VARCHAR, '$phone'::VARCHAR, $creditcardtype::INT, " .
    "'$creditcard'::VARCHAR, '$creditcardexpiration'::VARCHAR, '$password'::VARCHAR, " .
    "$age::SMALLINT, $income::INT, '$gender'::VARCHAR);";

  $result = pg_query($link_id,$new_customer_proc_call);
  if (!$result) {
    die("new_customer procedure failed: " . pg_last_error($link_id));
  }

  $row = pg_fetch_row($result);
  $customerid = $row[0];  // First OUT parameter
  $username = $row[1];    // Second OUT parameter
