...
UNION ALL SELECT 'OLD_ORDERS_COUNT', COUNT(*) FROM ORDERS{store_number} WHERE ORDERDATE < CURRENT_DATE
...
UNION ALL SELECT 'MAX_PROD_ID', COALESCE(MAX(PROD_ID), 0) FROM PRODUCTS{store_number}
...
BEGIN
    ...
    RAISE NOTICE 'Expected: After test, PROD_ID values divisible by {popular_modulo} should appear in top sales';
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Baseline metrics saved to VALIDATION_METRICS_{store_number} table';
