...
\\c ds3;
 ...
 CREATE TABLE CUSTOMERS$k
  ...
  -- Trigger function to audit MERGE operations on REVIEWS_HELPFULNESS
  CREATE OR REPLACE FUNCTION audit_helpfulness_merge$k()
  RETURNS TRIGGER AS \$\$
  BEGIN
    IF (TG_OP = 'INSERT') THEN
      ...
      VALUES
        ('REVIEWS_HELPFULNESS', 'INSERT', NEW.REVIEW_HELPFULNESS_ID, NEW.REVIEW_ID, NEW.CUSTOMERID, NULL, NEW.HELPFULNESS);
      RETURN NEW;
    ELSIF (TG_OP = 'UPDATE') THEN
      INSERT INTO MERGE_AUDIT$k
        (TABLE_NAME, OPERATION, REVIEW_HELPFULNESS_ID, REVIEW_ID, CUSTOMERID, OLD_HELPFULNESS, NEW_HELPFULNESS)
      VALUES
        ('REVIEWS_HELPFULNESS', 'UPDATE', NEW.REVIEW_HELPFULNESS_ID, NEW.REVIEW_ID, NEW.CUSTOMERID, OLD.HELPFULNESS, NEW.HELPFULNESS);
      RETURN NEW;
    END IF;
    RETURN NULL;
  END;
  ...
  -- Create trigger on REVIEWS_HELPFULNESS table
  CREATE TRIGGER audit_helpfulness_merge_trigger$k
  AFTER INSERT OR UPDATE ON REVIEWS_HELPFULNESS$k
  FOR EACH ROW
  EXECUTE FUNCTION audit_helpfulness_merge$k();
