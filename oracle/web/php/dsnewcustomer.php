...
<?php
/*
 * DVD Store New Customer PHP Oracle Page - dsnewcustomer.php
 *
 * Copyright (C) 2005 Dell, Inc. <davejaffe7@gmail.com> and <tmuirhead@vmware.com>
 *
 * Prompts for new customer data; creates new entry in Oracle DVD Store CUSTOMERS table
 *
...
// Username is generated by stored procedure, so only check required fields (not username)
if (!( empty($firstname) OR empty($lastname) OR empty($address1) OR empty($city) OR empty($country)
  ...
  // Handle empty fields - Oracle expects NULL for numeric fields
  $zip_val = empty($zip) ? NULL : $zip;
  $age_val = empty($age) ? NULL : intval($age);
  $income_val = empty($income) ? NULL : intval($income);
  $creditcardtype_val = empty($creditcardtype) ? NULL : intval($creditcardtype);

  // Call NEW_CUSTOMER stored procedure
  // Signature: (firstname, lastname, address1, address2, city, state, zip, country, region, email, phone,
  //             creditcardtype, creditcard, creditcardexpiration, username_out, password, age, income, gender, customerid_out)
  $sql = "BEGIN DS3.NEW_CUSTOMER$storenum(
    ...
    :username_out, :password, :age, :income, :gender, :customerid_out
  ); END;";

  $stmt = oci_parse($conn, $sql);

  // Bind input parameters
  oci_bind_by_name($stmt, ':firstname', $firstname, 50);
  oci_bind_by_name($stmt, ':lastname', $lastname, 50);
  oci_bind_by_name($stmt, ':address1', $address1, 50);
  oci_bind_by_name($stmt, ':address2', $address2, 50);
  oci_bind_by_name($stmt, ':city', $city, 50);
  oci_bind_by_name($stmt, ':state', $state, 50);
  oci_bind_by_name($stmt, ':zip', $zip_val, 10);
  oci_bind_by_name($stmt, ':country', $country, 50);
  oci_bind_by_name($stmt, ':region', $region, -1, SQLT_INT);
  oci_bind_by_name($stmt, ':email', $email, 50);
  oci_bind_by_name($stmt, ':phone', $phone, 16);
  oci_bind_by_name($stmt, ':creditcardtype', $creditcardtype_val, -1, SQLT_INT);
  oci_bind_by_name($stmt, ':creditcard', $creditcard, 16);
  oci_bind_by_name($stmt, ':creditcardexpiration', $creditcardexpiration, 7);
  oci_bind_by_name($stmt, ':password', $password, 8);
  oci_bind_by_name($stmt, ':age', $age_val, -1, SQLT_INT);
  oci_bind_by_name($stmt, ':income', $income_val, -1, SQLT_INT);
  oci_bind_by_name($stmt, ':gender', $gender, 1);

  // Bind output parameters
  oci_bind_by_name($stmt, ':username_out', $username_out, 24);
  ...
  if (!$result) {
    $e = oci_error($stmt);
    die("new_customer procedure failed: " . htmlentities($e['message'], ENT_QUOTES));
  }
  ...
  echo "<P>Your username is: <strong>" . htmlspecialchars($username_out) . "</strong></P>\n";
  echo "<P>Click below to begin shopping</P>\n";
  echo "<FORM ACTION='./dsbrowse.php' METHOD=GET>\n";
  echo "<INPUT TYPE=HIDDEN NAME=customerid VALUE=$customerid_out>\n";
