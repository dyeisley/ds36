 ...
 *
 * Gets Reviews of products in Oracle DVD store by product id, date, and star ranking
 *
...
if (!empty($getreviewtype))
  ...
  switch ($getreviewtype)
    {
    case "noorder":
      $sql = "BEGIN DS3.GET_PROD_REVIEWS$storenum(:p_prod_id, :p_batch_size); END;";
      $stmt = oci_parse($conn, $sql);
      oci_bind_by_name($stmt, ':p_prod_id', $productid, -1, SQLT_INT);
      oci_bind_by_name($stmt, ':p_batch_size', $limit_num, -1, SQLT_INT);
      break;
    case "date":
      $sql = "BEGIN DS3.GET_PROD_REVIEWS_BY_DATE$storenum(:p_prod_id, :p_batch_size); END;";
      $stmt = oci_parse($conn, $sql);
      oci_bind_by_name($stmt, ':p_prod_id', $productid, -1, SQLT_INT);
      oci_bind_by_name($stmt, ':p_batch_size', $limit_num, -1, SQLT_INT);
      break;
    case "star":
      $sql = "BEGIN DS3.GET_PROD_REVIEWS_BY_STARS$storenum(:p_prod_id, :p_stars, :p_batch_size); END;";
      $stmt = oci_parse($conn, $sql);
      oci_bind_by_name($stmt, ':p_prod_id', $productid, -1, SQLT_INT);
      oci_bind_by_name($stmt, ':p_stars', $review_stars, -1, SQLT_INT);
      oci_bind_by_name($stmt, ':p_batch_size', $limit_num, -1, SQLT_INT);
      break;
    }
