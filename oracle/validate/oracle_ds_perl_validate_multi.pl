#!/usr/bin/perl
# oracle_ds_perl_validate_multi.pl
# Script to generate and execute validation SQL for multiple stores
# Syntax: perl oracle_ds_perl_validate_multi.pl <server> <num_stores> <pre_test|post_test> [generate|execute]
...
# Validate arguments
if (!defined $oracletarget || !defined $numberofstores || !defined $mode) {
    print "Usage: perl oracle_ds_perl_validate_multi.pl <server> <num_stores> <pre_test|post_test> [generate|execute] [popular_modulo]\n";
    print "\nExamples:\n";
    print "  perl oracle_ds_perl_validate_multi.pl localhost 3 pre_test generate\n";
    print "  perl oracle_ds_perl_validate_multi.pl localhost 3 pre_test execute\n";
    print "  perl oracle_ds_perl_validate_multi.pl localhost 3 pre_test both 1000\n";
    print "  perl oracle_ds_perl_validate_multi.pl localhost 3 pre_test (defaults to both, modulo 10000)\n";
    exit 1;
}
...
# 2. DIRECTORY SETUP
my $oracletargetdir = $oracletarget;
$oracletargetdir =~ s/\\//;  # Remove backslashes for directory name
make_path($oracletargetdir);  # Cross-platform directory creation
...
if ($action eq 'generate' || $action eq 'both') {
    ...
    foreach my $k (1 .. $numberofstores) {
        my $output_file = "$oracletargetdir${pathsep}oracle_validate_${mode}${k}.sql";
...
if ($action eq 'execute' || $action eq 'both') {
    ...
    my $first_file = "$oracletargetdir${pathsep}oracle_validate_${mode}1.sql";
    if (!-f $first_file) {
    ...
    foreach my $k (1 .. $numberofstores) {
        ...
        system("sqlplus -S 'sys/oracle\@$oracletarget as sysdba'  \@$output_file");
    }
