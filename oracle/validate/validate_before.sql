...
BEGIN
    ...
    FROM user_tables
    WHERE table_name = 'VALIDATION_METRICS_{store_number}';

    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE TABLE VALIDATION_METRICS_{store_number} (
...
BEGIN
    SELECT COUNT(*) INTO v_count
    FROM user_tables
    WHERE table_name = 'VALIDATION_TOP_REVIEWS_{store_number}';

    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE TABLE VALIDATION_TOP_REVIEWS_{store_number} (
...
BEGIN
    SELECT COUNT(*) INTO v_count
    FROM user_tables
    WHERE table_name = 'VALIDATION_POPULAR_REVIEWS_{store_number}';

    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE TABLE VALIDATION_POPULAR_REVIEWS_{store_number} (
            prod_id NUMBER NOT NULL,
...
SELECT TABLENAME, ROWCOUNT FROM (
    SELECT 'CUSTOMERS' AS TABLENAME, COUNT(*) AS ROWCOUNT FROM DS3.CUSTOMERS{store_number}
    UNION ALL
    SELECT 'CUST_HIST', COUNT(*) FROM DS3.CUST_HIST{store_number}
    UNION ALL
    SELECT 'PRODUCTS', COUNT(*) FROM DS3.PRODUCTS{store_number}
    UNION ALL
    SELECT 'ORDERS', COUNT(*) FROM DS3.ORDERS{store_number}
    UNION ALL
    SELECT 'ORDERLINES', COUNT(*) FROM DS3.ORDERLINES{store_number}
    UNION ALL
    SELECT 'INVENTORY', COUNT(*) FROM DS3.INVENTORY{store_number}
    UNION ALL
    SELECT 'REVIEWS', COUNT(*) FROM DS3.REVIEWS{store_number}
    UNION ALL
    SELECT 'REVIEWS_HELPFULNESS', COUNT(*) FROM DS3.REVIEWS_HELPFULNESS{store_number}
    UNION ALL
    SELECT 'MEMBERSHIP', COUNT(*) FROM DS3.MEMBERSHIP{store_number}
    UNION ALL
    SELECT 'REORDER', COUNT(*) FROM DS3.REORDER{store_number}
)
ORDER BY TABLENAME;

-- Save table counts to metrics table
INSERT INTO VALIDATION_METRICS_{store_number} (metric_name, metric_value)
SELECT 'CUSTOMERS_COUNT', COUNT(*) FROM DS3.CUSTOMERS{store_number}
UNION ALL SELECT 'CUST_HIST_COUNT', COUNT(*) FROM DS3.CUST_HIST{store_number}
UNION ALL SELECT 'PRODUCTS_COUNT', COUNT(*) FROM DS3.PRODUCTS{store_number}
UNION ALL SELECT 'ORDERS_COUNT', COUNT(*) FROM DS3.ORDERS{store_number}
UNION ALL SELECT 'OLD_ORDERS_COUNT', COUNT(*) FROM DS3.ORDERS{store_number} WHERE ORDERDATE < TRUNC(SYSDATE)
UNION ALL SELECT 'ORDERLINES_COUNT', COUNT(*) FROM DS3.ORDERLINES{store_number}
UNION ALL SELECT 'INVENTORY_COUNT', COUNT(*) FROM DS3.INVENTORY{store_number}
UNION ALL SELECT 'REVIEWS_COUNT', COUNT(*) FROM DS3.REVIEWS{store_number}
UNION ALL SELECT 'REVIEWS_HELPFULNESS_COUNT', COUNT(*) FROM DS3.REVIEWS_HELPFULNESS{store_number}
UNION ALL SELECT 'MEMBERSHIP_COUNT', COUNT(*) FROM DS3.MEMBERSHIP{store_number}
UNION ALL SELECT 'REORDER_COUNT', COUNT(*) FROM DS3.REORDER{store_number}
UNION ALL SELECT 'MAX_PROD_ID', NVL(MAX(PROD_ID), 0) FROM DS3.PRODUCTS{store_number}
UNION ALL SELECT 'SILVER_MEMBERS_COUNT', COUNT(*) FROM DS3.MEMBERSHIP{store_number} WHERE MEMBERSHIPTYPE = 2
UNION ALL SELECT 'GOLD_MEMBERS_COUNT', COUNT(*) FROM DS3.MEMBERSHIP{store_number} WHERE MEMBERSHIPTYPE = 3;
COMMIT;

-- Create full membership snapshot to track all changes across test run
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE MEMBERSHIP_SNAPSHOT_{store_number}';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
...
FROM DS3.MEMBERSHIP{store_number};

CREATE INDEX idx_snapshot_customerid ON MEMBERSHIP_SNAPSHOT_{store_number}(CUSTOMERID);
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Baseline metrics saved to VALIDATION_METRICS_{store_number} table');
