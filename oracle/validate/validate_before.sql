...
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE MEMBERSHIP_SNAPSHOT';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
...
CREATE TABLE MEMBERSHIP_SNAPSHOT AS
SELECT CUSTOMERID, MEMBERSHIPTYPE, EXPIREDATE
FROM DS3.MEMBERSHIP1;

CREATE INDEX idx_snapshot_customerid ON MEMBERSHIP_SNAPSHOT(CUSTOMERID);
