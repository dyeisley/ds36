...
UNION ALL SELECT 'OLD_ORDERS_COUNT', COUNT(*) FROM DS3.ORDERS1 WHERE ORDERDATE < TRUNC(SYSDATE)
...
UNION ALL SELECT 'SILVER_MEMBERS_COUNT', COUNT(*) FROM DS3.MEMBERSHIP1 WHERE MEMBERSHIPTYPE = 2
UNION ALL SELECT 'GOLD_MEMBERS_COUNT', COUNT(*) FROM DS3.MEMBERSHIP1 WHERE MEMBERSHIPTYPE = 3;
COMMIT;

-- Create sample of 10000 random members to track changes across test run
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE VALIDATION_MEMBERSHIP_SAMPLE';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -942 THEN
            RAISE;
...
CREATE TABLE VALIDATION_MEMBERSHIP_SAMPLE AS
