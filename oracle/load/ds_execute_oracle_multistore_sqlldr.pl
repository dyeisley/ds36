...
foreach my $k (1 .. $numberofstores){
	system ("start remote_oracle_ds_membership_sqlldr$k.bat");
	}

chdir("$base_dir\\prod\\$oracletargetdir");
foreach my $k (1 .. $numberofstores){
	system ("start remote_oracle_ds_prod_sqlldr$k.bat");
	system ("start remote_oracle_ds_inv_sqlldr$k.bat");
	}
	
	chdir("$base_dir\\reviews\\$oracletargetdir");
foreach my $k (1 .. $numberofstores){
	system ("start remote_oracle_ds_reviews_sqlldr$k.bat");
	}
	
	chdir("$base_dir\\orders\\$oracletargetdir");
foreach my $k (1 .. $numberofstores){
	system ("start remote_oracle_ds_orders_sqlldr$k.bat");
	system ("start remote_oracle_ds_orderlines_sqlldr$k.bat");
	system ("start remote_oracle_ds_cust_hist_sqlldr$k.bat");
	}
	
	chdir("$base_dir\\cust\\$oracletargetdir");
foreach my $k (1 .. $numberofstores){
	system ("start remote_oracle_ds_cust_sqlldr$k.bat");
	}
