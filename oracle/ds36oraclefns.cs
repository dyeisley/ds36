...
namespace ds2xdriver
{
  ...
  public class ds2Interface
  {
    ...
    OracleCommand Login, New_Customer, Browse_By_Category, Browse_By_Actor, Browse_By_Title, Browse_By_Membership, Browse_By_Vector, New_Product, Purchase;
    ...
    public ds2Interface(int ds2interfaceid, string target_name, int target_store)
    {
      ...
      //Browse_By_Vector
      Browse_By_Vector = new OracleCommand("BROWSE_BY_VECTOR" + target_store_number, objConn);
      Browse_By_Vector.CommandType = CommandType.StoredProcedure;
      Browse_By_Vector.Parameters.Add("p_batch_size_in", OracleDbType.Int32);
      Browse_By_Vector.Parameters.Add("p_vector_text", OracleDbType.Clob);
      ...
      Purge_Old_Orders = new OracleCommand("DS3.PurgeOldOrders" + target_store_number, objConn);
      Purge_Old_Orders.CommandType = CommandType.StoredProcedure;
      Purge_Old_Orders.Parameters.Add("p_batch_size", OracleDbType.Int32);
      Purge_Old_Orders.Parameters.Add("p_rows_affected", OracleDbType.Int32, ParameterDirection.Output);

      Upgrade_Membership = new OracleCommand("DS3.UpgradeMembership" + target_store_number, objConn);
    ...
    public int ds36purgeoldorders(int batchSize, ref double rt)
    {
      ...
      try
      {
        Purge_Old_Orders.ExecuteNonQuery();
        object result = Purge_Old_Orders.Parameters["p_rows_affected"].Value;
        return result != null && result != DBNull.Value ? Convert.ToInt32(result.ToString()) : 0;
      }
      catch (Exception e)
      {
    ...
    public int ds36upgrademembership(int batchSize, ref double rt)
    {
      ...
      try
      {
        Upgrade_Membership.ExecuteNonQuery();
        object result = Upgrade_Membership.Parameters["p_rows_upgraded"].Value;
        return result != null && result != DBNull.Value ? Convert.ToInt32(result.ToString()) : 0;
      }
      catch (Exception e)
      {
    ...
    {
      string sqlFilePath = $"validate/{targetServer}/oracle_validate_post_test{storeNumber}.sql";
      if (!File.Exists(sqlFilePath))
      {
        ...
        Console.WriteLine($"  perl oracle_ds_perl_validate_multi.pl {targetServer} {storeNumber} post_test generate");
        return;
      }
